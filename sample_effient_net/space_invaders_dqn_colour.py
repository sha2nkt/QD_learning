...
def update_parameters(online_net, target_net, batch):
    ...
    current_q_values = online_net(Variable(torch.from_numpy(np.float32(np.array([r for r in curr_states]))).cuda()))

    next_state_q_values = torch.max(target_net(Variable(torch.from_numpy(np.float32(np.array([r for r in next_states]))).cuda())), 1)
...
for ie in range(num_episodes):
    ...
    for j in range(maxSteps):
        ...
        epsilon = EPSILON * max(1. - ((s_iter * 1.0) / num_episodes), EPSILON_MIN)
    ...
    if ie % 10 == 0:
        torch.save({'target_net': target_net, 'online_net': online_net}, 'grid_dqn_color.pth')
        np.save('space_invaders_dqn_color_stats.npy', eval_stats)

torch.save({'target_net':target_net,'online_net':online_net},'grid_dqn_color_end.pth')
