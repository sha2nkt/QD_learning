...
class Agent():
    ...
    def test(self):
        ...
        # prevState = self.env.reset()
        # prevState = prevState[:,:,0]
        # a = np.random.randint(0,5)
        # curState, r, d = self.env.step(a)
        # curState = curState[:,:,0]
        # a = np.random.randint(0,5)
        # nextState, r, d = self.env.step(a)
        # nextState = nextState[:,:,0]
        #
        # prevState = torch.from_numpy(prevState.reshape(-1,81)).type(torch.FloatTensor).cuda(async=True)
        # curState = torch.from_numpy(curState.reshape(-1,81)).type(torch.FloatTensor).cuda(async=True)
        # prevState = torch.autograd.Variable(prevState)
        # curState = torch.autograd.Variable(curState)
        #
        #
        # predState = self.model(prevState, curState)
        # sigmoid = nn.Sigmoid()
        # predState = sigmoid(predState)
        # predState = predState >= 0.2
        #
        # predState = predState.data.cpu().numpy().reshape(9, 9)
        # nextState = nextState.astype(np.int).reshape(9, 9)
        # pdb.set_trace()
        # acc = np.sum(predState == nextState)*1.0 / (predState.shape[0] * predState.shape[1])
        #         sum_acc += acc
        #         count_acc += 1

        # print(predState.data.cpu().numpy().reshape(9,9))
